<template>
  <div>
    <TypeNav />
    <div class="main">
      <div class="py-container">
        <!--bread-->
        <div class="bread">
          <ul class="fl sui-breadcrumb">
            <li>
              <a href="#">全部结果</a>
            </li>
          </ul>
          <ul class="fl sui-tag">

            <!-- 三种面包屑 -->
            <li class="with-x" v-if="searchParams.categoryName">
              {{ searchParams.categoryName
              }}<i @click="removeCategoryName">×</i>
            </li>

            <!-- 如果存在searchParams.categoryName则显示 -->
            <!-- 点击删除 -->
            <li class="with-x" v-if="searchParams.keyword">
              {{ searchParams.keyword }}<i @click="removeKeyWord">×</i>
            </li>

            <li class="with-x" v-if="searchParams.trademark">
              {{ searchParams.trademark.split(':')[1] }}<i @click="removeTradeMark">×</i>
            </li>

            <!-- 遍历数组生成面包屑 -->
            <li class="with-x" v-for="p in searchParams.props" :key="p.split(':')[0]">
                {{p.split(':')[1]}}
                <i @click="removeAttrInfo(p)">×</i>
                <!-- 这里也可以传index  -->
            </li>



          </ul>
        </div>

        <!--selector-->
        <SearchSelector />

        <!--details-->
        <div class="details clearfix">
          <!-- 顶栏 -->
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active:searchParams.order.indexOf('1')!=-1}" @click = "changeOrder('1')">
                  <!-- 显示样式 和 传递默认参数-->
                  <a href="#">综合
                    <span v-show = "searchParams.order.indexOf('1')!=-1"
                    class = "iconfont" 
                    :class = "{'icon-jiantou_xiangxia':searchParams.order.indexOf('desc')!=-1,'icon-jiantou_xiangshang':searchParams.order.indexOf('asc')!=-1}" >
                    </span >
                    <!-- 如果order有desc,class就是icon-jiantou_xiangxia
                    有asc,class就是icon-jiantou_xiangshang -->
                  </a>
                </li>
                <li :class="{active:searchParams.order.indexOf('2')!=-1}" @click = "changeOrder('2')">
                  <!-- 显示样式 -->
                  <a href="#">价格
                    <span v-show = "searchParams.order.indexOf('2')!=-1"
                    class = "iconfont" 
                    :class = "{'icon-jiantou_xiangxia':searchParams.order.indexOf('desc')!=-1,'icon-jiantou_xiangshang':searchParams.order.indexOf('asc')!=-1}" >
                    </span >
                  </a>
                </li>
              </ul>
            </div>
          </div>

          <!-- 产品 -->
          <div class="goods-list">
            <ul class="yui3-g">
              <li class="yui3-u-1-5" v-for="p in goodsList" :key="p.id">
                <div class="list-wrap">
                  <div class="p-img">
                    <router-link :to="{name:'Details',params:{id:p.id}}">
                      <!-- <img :src="p.defaultImg"/> -->
                      <img v-lazy="p.defaultImg"/>
                    </router-link>
                  </div>
                  <div class="price">
                    <strong>
                      <em>¥</em>&nbsp;
                      <i>{{ p.price }}</i>
                    </strong>
                  </div>
                  <div class="attr">
                    <a
                      target="_blank"
                      href="item.html"
                      title="促销信息，下单即赠送三个月CIBN视频会员卡！【小米电视新品4A 58 火爆预约中】"
                      >{{ p.title }}</a
                    >
                  </div>
                  <div class="commit">
                    <i class="command">已有<span>2000</span>人评价</i>
                  </div>
                  <div class="operate">
                    <a
                      href="success-cart.html"
                      target="_blank"
                      class="sui-btn btn-bordered btn-danger"
                      >加入购物车</a
                    >
                    <a href="javascript:void(0);" class="sui-btn btn-bordered"
                      >收藏</a
                    >
                  </div>
                </div>
              </li>
            </ul>
          </div>

        </div>
      </div>
    </div>
    <Pagination :pageNo = "searchParams.pageNo" :pageSize="searchParams.pageSize" total="91" continues="5"/>
     <!-- <Pagination pageNo = "5" pageSize="3" total="91" continues="5"/> -->
  </div>
</template>

<script>
import { mapState, mapGetters } from "vuex";
import SearchSelector from "./SearchSelector/SearchSelector";
export default {
  name: "Search",
  components: {
    SearchSelector,
  },
  data() {
    return {
      //初始化参数
      searchParams: {
        //产品相应的id
        category1Id: "",
        category2Id: "",
        category3Id: "",
        //产品的名字
        categoryName: "",
        //搜索的关键字
        keyword: "",
        //排序:初始状态应该是综合且降序
        order: "1:desc",
        //第几页
        pageNo: 1,
        //每一页展示条数
        pageSize: 10,
        //平台属性的操作
        props: [],
        //品牌
        trademark: "",
      },
    };
  },
  beforeMount() {
    // Object.assign(this.searchParams,this.$route.query,this.$route.params)
    //合并参数
  },
  mounted() {
    // this.$store.dispatch('getSearchList',{})
    // 不能放在这



    //全局总线事件 SearchSelector传的参数
    this.$bus.$on('trademark',this.trademark)

    this.$bus.$on('attrlist',this.attrlist)

    //分页器跳页
    this.$bus.$on('changePageNo',this.changePageNo)
  },
  computed: {
    // ...mapState({
    //   searchList : (state)=>state.search.searchList
    // })
    //这里由于模块化 拿数据比较复杂

    //getters没划分模块 拿数据比较简单
    ...mapGetters(["goodsList"]),
  },
  methods: {
    // 发送数据
    getData() {
      this.searchParams.category1Id = "";
      this.searchParams.category2Id = "";
      this.searchParams.category3Id = "";
      //置空 防止上一次的数据干扰
      Object.assign(this.searchParams, this.$route.query, this.$route.params);
      //合并参数
      this.$store.dispatch("getSearchList", this.searchParams);
      //axios请求
    },

    //删除菜单栏的面包屑
    removeCategoryName() {
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
      this.searchParams.categoryName = undefined; //优化
      //置空

      this.$store.dispatch("getSearchList", this.searchParams);
      //重新获取 删掉面包屑后服务器返回的数据

      if (this.$route.params) {
        this.$router.push({ name: "Search", params: this.$route.params });
      }
      //作用只是改变url地址 让它变成新的
      //地址栏也需要需改：进行路由跳转(现在的路由跳转只是跳转到自己这里)
      //严谨：本意是删除query，如果路径当中出现params不应该删除，路由跳转的时候应该带着
    },

    //删除搜索栏的面包屑
    removeKeyWord() {
        this.searchParams.keyword = undefined;
        this.$store.dispatch("getSearchList", this.searchParams);
        if(this.$route.query){
          this.$router.push({ name: "Search", query: this.$route.query });
        }


        this.$bus.$emit('clearSearch')
        //清除header的搜索栏
    },

    //点击子组件的品牌生成面包屑 是一个全局总线的回调
    trademark(p){
      // console.log('@',p);
      this.searchParams.trademark = `${p.tmId}:${p.tmName}`
      this.$store.dispatch("getSearchList", this.searchParams)
    },

    //移除品牌生成的面包屑
    removeTradeMark(){
      this.searchParams.trademark = ""
      this.$store.dispatch("getSearchList", this.searchParams)
    },

    //点击改变排序
    changeOrder(flag){
      let first = this.searchParams.order.split(':')[0]//order初始参数1
      let last = this.searchParams.order.split(':')[1]//order初始参数2
      
      //点击相同的按钮 箭头改变为相反
      if(flag == first){
        if(last == "asc"){
          this.searchParams.order = flag+":desc"
        }
        else{
          this.searchParams.order = flag+":asc"
        }
      }
      //点击不同的按钮 箭头默认向下 且改变参数1
      else{
        this.searchParams.order = flag+":desc"
      }

      
      //重新向服务器发送请求
      this.$store.dispatch("getSearchList", this.searchParams)
    },

    //全局总线的回调函数 发送属性面包屑的axios请求
    attrlist(q,p){
      let str = p.attrId + ':' + q + ':' + p.attrName

      if( this.searchParams.props.indexOf(str) == -1)//如果没有
      this.searchParams.props.push(str)
      //加入props

      //重新向服务器发送请求
      this.$store.dispatch("getSearchList", this.searchParams)
    },

    //删除属性面包屑
    removeAttrInfo(p){
      //点击事件传递参数p 也可以直接传index更简单
      let index = this.searchParams.props.indexOf(p)
      //找到p在数组的位置
      this.searchParams.props.splice(index,1)
      //删除

      this.$store.dispatch("getSearchList", this.searchParams)
      //重新向服务器发送请求
    },

    //分页器跳页
    changePageNo(page){
      // 获取全局总线发送的跳转页数
      this.searchParams.pageNo = page

      this.$store.dispatch("getSearchList", this.searchParams)
      //重新向服务器发送请求
    }
  },
  watch: {
    $route: {
      immediate: true,
      handler() {
        this.getData();
      },
    },
  },
};
</script>

<style lang="less" scoped>
.main {
  margin: 10px 0;

  .py-container {
    width: 1200px;
    margin: 0 auto;

    .bread {
      margin-bottom: 5px;
      overflow: hidden;

      .sui-breadcrumb {
        padding: 3px 15px;
        margin: 0;
        font-weight: 400;
        border-radius: 3px;
        float: left;

        li {
          display: inline-block;
          line-height: 18px;

          a {
            color: #666;
            text-decoration: none;

            &:hover {
              color: #4cb9fc;
            }
          }
        }
      }

      .sui-tag {
        margin-top: -5px;
        list-style: none;
        font-size: 0;
        line-height: 0;
        padding: 5px 0 0;
        margin-bottom: 18px;
        float: left;

        .with-x {
          font-size: 12px;
          margin: 0 5px 5px 0;
          display: inline-block;
          overflow: hidden;
          color: #000;
          background: #f7f7f7;
          padding: 0 7px;
          height: 20px;
          line-height: 20px;
          border: 1px solid #dedede;
          white-space: nowrap;
          transition: color 400ms;
          cursor: pointer;

          i {
            margin-left: 10px;
            cursor: pointer;
            font: 400 14px tahoma;
            display: inline-block;
            height: 100%;
            vertical-align: middle;
          }

          &:hover {
            color: #28a3ef;
          }
        }
      }
    }

    .details {
      margin-bottom: 5px;

      .sui-navbar {
        overflow: visible;
        margin-bottom: 0;

        .filter {
          min-height: 40px;
          padding-right: 20px;
          background: #fbfbfb;
          border: 1px solid #e2e2e2;
          padding-left: 0;
          border-radius: 0;
          box-shadow: 0 1px 4px rgba(0, 0, 0, 0.065);

          .sui-nav {
            position: relative;
            left: 0;
            display: block;
            float: left;
            margin: 0 10px 0 0;

            li {
              float: left;
              line-height: 18px;

              a {
                display: block;
                cursor: pointer;
                padding: 11px 15px;
                color: #777;
                text-decoration: none;
              }

              &.active {
                a {
                  background: #e1251b;
                  color: #fff;
                }
              }
            }
          }
        }
      }

      .goods-list {
        margin: 20px 0;

        ul {
          display: flex;
          flex-wrap: wrap;

          li {
            height: 100%;
            width: 20%;
            margin-top: 10px;
            line-height: 28px;

            .list-wrap {
              .p-img {
                padding-left: 15px;
                width: 215px;
                height: 255px;

                a {
                  color: #666;

                  img {
                    max-width: 100%;
                    height: auto;
                    vertical-align: middle;
                  }
                }
              }

              .price {
                padding-left: 15px;
                font-size: 18px;
                color: #c81623;

                strong {
                  font-weight: 700;

                  i {
                    margin-left: -5px;
                  }
                }
              }

              .attr {
                padding-left: 15px;
                width: 85%;
                overflow: hidden;
                margin-bottom: 8px;
                min-height: 38px;
                cursor: pointer;
                line-height: 1.8;
                display: -webkit-box;
                -webkit-box-orient: vertical;
                -webkit-line-clamp: 2;

                a {
                  color: #333;
                  text-decoration: none;
                }
              }

              .commit {
                padding-left: 15px;
                height: 22px;
                font-size: 13px;
                color: #a7a7a7;

                span {
                  font-weight: 700;
                  color: #646fb0;
                }
              }

              .operate {
                padding: 12px 15px;

                .sui-btn {
                  display: inline-block;
                  padding: 2px 14px;
                  box-sizing: border-box;
                  margin-bottom: 0;
                  font-size: 12px;
                  line-height: 18px;
                  text-align: center;
                  vertical-align: middle;
                  cursor: pointer;
                  border-radius: 0;
                  background-color: transparent;
                  margin-right: 15px;
                }

                .btn-bordered {
                  min-width: 85px;
                  background-color: transparent;
                  border: 1px solid #8c8c8c;
                  color: #8c8c8c;

                  &:hover {
                    border: 1px solid #666;
                    color: #fff !important;
                    background-color: #666;
                    text-decoration: none;
                  }
                }

                .btn-danger {
                  border: 1px solid #e1251b;
                  color: #e1251b;

                  &:hover {
                    border: 1px solid #e1251b;
                    background-color: #e1251b;
                    color: white !important;
                    text-decoration: none;
                  }
                }
              }
            }
          }
        }
      }

      .page {
        width: 733px;
        height: 66px;
        overflow: hidden;
        float: right;

        .sui-pagination {
          margin: 18px 0;

          ul {
            margin-left: 0;
            margin-bottom: 0;
            vertical-align: middle;
            width: 490px;
            float: left;

            li {
              line-height: 18px;
              display: inline-block;

              a {
                position: relative;
                float: left;
                line-height: 18px;
                text-decoration: none;
                background-color: #fff;
                border: 1px solid #e0e9ee;
                margin-left: -1px;
                font-size: 14px;
                padding: 9px 18px;
                color: #333;
              }

              &.active {
                a {
                  background-color: #fff;
                  color: #e1251b;
                  border-color: #fff;
                  cursor: default;
                }
              }

              &.prev {
                a {
                  background-color: #fafafa;
                }
              }

              &.disabled {
                a {
                  color: #999;
                  cursor: default;
                }
              }

              &.dotted {
                span {
                  margin-left: -1px;
                  position: relative;
                  float: left;
                  line-height: 18px;
                  text-decoration: none;
                  background-color: #fff;
                  font-size: 14px;
                  border: 0;
                  padding: 9px 18px;
                  color: #333;
                }
              }

              &.next {
                a {
                  background-color: #fafafa;
                }
              }
            }
          }

          div {
            color: #333;
            font-size: 14px;
            float: right;
            width: 241px;
          }
        }
      }
    }
  }
}
</style>
<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="p in userAddress" :key="p.id">
        <span class="username" :class="{ selected: p.isDefault == 1 }">{{
          p.consignee
        }}</span>
        <p @click="changeAddress(p)">
          <span class="s1">{{ p.fullAddress }}</span>
          <span class="s2">{{ p.phoneNum }}</span>
          <span class="s3" v-show="p.isDefault == 1">默认地址</span>
        </p>
      </div>
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="p in trade.detailArrayList"
          :key="p.skuId"
        >
          <li>
            <img :src="p.imgUrl" alt="" style="width: 100px; height: 100px" />
          </li>
          <li>
            <p>{{ p.skuName }}</p>
            <h4>7天无理由退货</h4>
    ...
    <div class="sub clearFix">
      <a href="javascript:void(0);" class="subBtn" @click="subOrder"
        >提交订单</a
      >
    </div>
  </div>
...
import { mapState } from 'vuex'
  export default {
    ...
    computed:{
      ...
      //选择的默认地址 计算属性
      userDefaultAddress(){
        return this.userAddress.find((p)=>{
    ...
    methods:{
      ...
      async subOrder(){
        ...
        let result = await this.$API.reqSubOrder(tradeNo,params)
        //请求服务器接口

        this.$router.push('/pay?orderId='+123)
        //项目出错了 用点小技巧
